...
x=str(form.getvalue("on_date"))
y1, m1, d1 = map(int, x.split('-'))
...
<head>
	...
	<body><center>
	<div id="dd" id="txt"><h1>Plants to be Watered</h1></div><br><br>
...
<thead>
  ...
  ''')
for i in r:
	d2, m2, y2 = map(int, i[2].split('-'))
	diff=datetime.datetime(y1, m1, d1)-datetime.datetime(y2, m2, d2)
	days=diff.days
	if (days%i[4]==0):
		print('  <tr>')
		...
		print('    <td id="txt" align="center">%d</td>'%i[4])
